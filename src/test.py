...
def generate_provider_url(provider_name, content_id, content_type):
    base_urls = {
        'Netflix': f'https://www.netflix.com/title/{content_id}',
        'Amazon Prime Video': f'https://www.amazon.com/dp/{content_id}',
        # You can add more platforms and their URL formats here
    }
    return base_urls.get(provider_name, f'https://www.themoviedb.org/{content_type}/{content_id}')
...
def main():
    ...
    region = 'IN'  # Change this to your region code if needed
    count = 10  # Number of movies/shows to retrieve per category

    os.makedirs('data/new', exist_ok=True)  # Create data directory if it doesn't exist
    ...
    for content_type in content_types:
        ...
        for content in trending_content:
            ...
            # Get cast members
            cast = [actor['name'] for actor in content_details.get('credits', {}).get('cast', [])][:5]  # Limit to top 5 actors
            
            # Get platforms where the content is available and generate URLs
            platforms = []
            platform_urls = {}
            for provider in content_providers:
                provider_name = provider['provider_name']
                platforms.append(provider_name)
                platform_urls[provider_name] = generate_provider_url(provider_name, content_id, content_type)
            ...
            content_info = {
                ...
                'Platform URLs': platform_urls if platform_urls else 'No URL information available'
            }

            content_data.append(content_info)
        
        # Save each content type (movies and TV series) to a separate JSON file
        filename = f'data/new/trending_{content_type}.json'
